...
export function bottleIcon(w: Wine, h = 64, known = true): string {
  ...
  const body = known ? (clear ? LIQUID[w.liquid].color : g.tint) : "#3b322d";
...
${known ? `<rect x="${(cx - capR).toFixed(1)}" y="${capTop.toFixed(1)}" width="${(capR * 2).toFixed(1)}" height="${(capY - capTop).toFixed(1)}" rx="1" fill="${w.capsule.color}"/>` : ""}
${known ? `<rect x="${(cx - def.R * scale * 0.85).toFixed(1)}" y="${Y(w.label.y1).toFixed(1)}" width="${(def.R * scale * 1.7).toFixed(1)}" height="${((w.label.y1 - w.label.y0) * scale).toFixed(1)}" fill="rgba(245,236,215,0.9)"/>` : ""}
</svg>`;
  cache.set(key, svg);
  return svg;
}
